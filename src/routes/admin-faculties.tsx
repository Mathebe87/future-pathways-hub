...
type AdminUniversity = { id: string; name: string; shortCode: string };
...
function AdminFaculties() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => api.get<Faculty[]>(`/api/admin/faculties?universityId=${universityId}`),
  });
  const faculties = data ?? [];
  const { pageItems, page, setPage, totalPages, total, from, to } = usePagination(faculties, 10);
  const invalidate = () => qc.invalidateQueries({ queryKey: ["admin-faculties", universityId] });

  const createMut = useMutation({
    mutationFn: (n: string) => api.post("/api/admin/faculties", { universityId, name: n }),
    onSuccess: () => {
      invalidate();
      closeForm();
    },
  ...
  const renameMut = useMutation({
    mutationFn: ({ id, n }: { id: string; n: string }) =>
      api.patch(`/api/admin/faculties/${id}`, { name: n }),
    onSuccess: () => {
      invalidate();
      closeForm();
    },
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => api.del(`/api/admin/faculties/${id}`),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        eyebrow="Content"
        title="Faculties"
        subtitle="Organise each university's programmes into faculties or schools."
        icon={Building2}
        tone="fuchsia"
        actions={
          <button className={btn.primary} onClick={openAdd} disabled={!universityId}>
            <Plus className="h-4 w-4" /> New faculty
          </button>
        }
      />

      <Panel className="mb-6" bodyClassName="max-w-md">
        <label className="mb-1.5 block text-xs font-medium text-muted-foreground">University</label>
      ...
      {!universityId ? (
        <Panel>
          <div className="py-12 text-center text-sm text-muted-foreground">
            Select a university above to view and manage its faculties.
          </div>
        </Panel>
      ) : (
        <Panel flush>
                    ...
                    {faculties.length === 0 && (
                      <tr>
                        <td colSpan={2} className="px-5 py-16 text-center text-sm text-muted-foreground">
                          No faculties yet. Add one to group this university's programmes.
                        </td>
                      </tr>
                    )}
                  </tbody>
